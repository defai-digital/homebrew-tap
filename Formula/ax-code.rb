...
class AxCode < Formula
  ...
  version "7.19.3"
  url "https://download.ax-code.com/releases/download/v7.19.3/ax-code-darwin-arm64.zip"
  sha256 "b6fdbd49576a9392900e0f8adfeeadeac792847c5b9ba01a927d76e7d6b77cba"
  license "Apache-2.0"
  ...
  AX_TUI_DYLIB = "node_modules/ax-tui/vendor/darwin-arm64/libaxtui.dylib"

  def install
    libexec.install Dir["*"]
    (bin/"ax-code").write <<~SH
      #!/bin/sh
      # Use the release launcher for atomic, runtime-specific Node branding.
      export AX_CODE_SYSTEM_NODE=1
      export PATH="#{formula_opt_bin("node")}:$PATH"
      exec "#{libexec}/bin/ax-code" "$@"
    SH
  ...
  post_install_steps do
    if_path_exists "node_modules/ax-tui/vendor/darwin-arm64/libaxtui.dylib.gz", base: :libexec do
      set_permissions "node_modules/ax-tui/vendor/darwin-arm64", "0755", base: :libexec, recursive: false
      run "/usr/bin/gunzip",
          args:           ["--", "{{libexec}}/node_modules/ax-tui/vendor/darwin-arm64/libaxtui.dylib.gz"],
