class MlxC < Formula
  desc "C API for MLX (defai-digital: depends on the NAX-fixed mlx)"
  ...
  # Deliberately no `bottle do` block: must always rebuild from source
  # against the tap-local `mlx` below, not homebrew-core's bottled mlx-c
  # (which links against homebrew-core's NAX-regressed mlx).
